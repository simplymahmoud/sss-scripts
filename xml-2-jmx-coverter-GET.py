...
if __name__ == '__main__':
	...
	for temp_count in range(main_loop_counter):
		...
		write_to_jmx(output, '        <stringProp name="ThreadGroup.ramp_time">0</stringProp>')
		...
		for url in urls[loop_start:loop_end]:
			...
			write_to_jmx(output, '        <HTTPSamplerProxy guiclass="HttpTestSampleGui" testclass="HTTPSamplerProxy" testname="GET %s" enabled="true">' % url_path)
			...
			write_to_jmx(output, '          <stringProp name="HTTPSampler.method">GET</stringProp>')
